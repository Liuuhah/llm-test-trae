...
class ChatCompressClient:
    def __init__(self):
        ...
        skills_root = os.path.join(project_root, '..', '.agents', 'skills')
        self.skill_manager = SkillManager(skills_root)
        self.skill_manager.scan_and_cache()  # 启动时扫描并缓存技能
        ...
        tools_list = [
            ...
            },
            {
                ...
                "function": {
                    ...
                    "description": "列出所有可用技能。当用户需要帮助但不确定使用哪个技能时调用。返回技能的名称和描述。",
                    "parameters": {
                        "type": "object",
                        "properties": {},
                        "required": []
            ...
            },
            {
                ...
                "function": {
                    ...
                    "description": "加载指定技能的详细内容。当用户请求需要使用特定技能时（如写通知、买保险等），先调用此工具获取技能指令，然后严格按照技能要求执行。",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "skill_name": {
                                "type": "string",
                                "description": "技能的唯一标识符（对应目录名），例如：'notice'、'insurance'"
                            }
                        },
                        "required": ["skill_name"]
                    }
    ...
    def execute_tool(self, tool_name, tool_args):
        ...
        if tool_name == "list_directory":
            ...
            return tools.list_available_skills()
        elif tool_name == "load_skill_content":
            return tools.load_skill_content(
...
总结："""
    ...
    def _send_single_stream(self, prompt, max_tokens, debug, is_first_round):
        ...
        messages_list = [
            {'role': 'system', 'content': f'''你是一个友好的AI助手，具有以下能力：
1. 文件操作：列出目录、读写文件、重命名、删除
2. 网页访问：使用 curl 工具访问网页
3. 知识库查询：当用户提到"文档仓库"、"文件仓库"、"知识库"、"查找文档"时，使用 anythingllm_query 工具查询本地文档
4. 动态技能：可以使用 list_available_skills 查看可用技能，使用 load_skill_content 加载特定技能的详细指令

重要提示：
- 如果用户询问关于公司内部文档、政策、项目资料等问题，优先使用 anythingllm_query 工具
- 如果用户请求需要使用特定技能（如写通知、买保险等），先调用 load_skill_content 获取技能指令，然后严格按照技能要求执行
- 如果用户询问通用知识，直接回答即可
- 工具调用失败时，尝试其他方法或告知用户
- 从知识库返回结果时，只展示与问题直接相关的内容，过滤掉明显不相关的文档

【可用技能列表】
{self.skill_manager.get_skill_json()}
...
- 不要翻译或解释工具返回的内容，直接展示即可'''}
    ...
    def _execute_pending_tool_call(self, tool_call_data, data, conn, headers, prompt):
        ...
        # 如果是加载技能，将技能内容作为 System 消息注入
        if normalized_tool_name == 'load_skill_content':
            skill_name = tool_args.get('skill_name', '')
            skill_content = tool_result if isinstance(tool_result, str) else json.dumps(tool_result, ensure_ascii=False)
            
            # 检查是否是错误信息
            if not skill_content.startswith('错误：'):
                system_message = f"请严格遵照以下 [{skill_name}] 技能的指令执行：\n\n{skill_content}"
                self.add_to_history('system', system_message)
                print(f"[技能加载] 已加载技能 '{skill_name}' 并注入到上下文")
