...
class Tools:
    ...
    def list_available_skills(self):
        ...
        Returns:
            str: JSON string containing name and description.
        """
        try:
            # 导入 SkillManager
            from skill_manager import SkillManager
            
            # 获取项目根目录下的 .agents/skills 路径
            import os
            project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
            skills_root = os.path.join(project_root, '.agents', 'skills')
            
            # 创建 SkillManager 实例并扫描
            manager = SkillManager(skills_root)
            skills_list = manager.scan_and_cache()
            
            # 返回 JSON 格式
            return json.dumps(skills_list, ensure_ascii=False, indent=2)
            
        except Exception as e:
            return json.dumps({"error": f"列出技能失败: {str(e)}"}, ensure_ascii=False)
    ...
    def load_skill_content(self, skill_name):
        ...
        Returns:
            str: The content of SKILL.md excluding the front matter.
        """
        try:
            # 导入 SkillManager
            from skill_manager import SkillManager
            ...
            # 创建 SkillManager 实例并加载内容
            manager = SkillManager(skills_root)
            content = manager.load_full_content(skill_name)
            
            return content
            
        except Exception as e:
            return f"错误：加载技能内容失败 - {str(e)}"
